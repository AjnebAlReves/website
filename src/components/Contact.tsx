...
const Contact: React.FC<ContactProps> = ({ isDark, toggleTheme }) => {
	const { t } = useI18n();
	// IDs únicos para campos y errores
	const idName = useId();
	const idEmail = useId();
	const idSubject = useId();
	const idMessage = useId();
	const idBotField = useId();
	const idNameError = useId();
	const idEmailError = useId();
	const idSubjectError = useId();
	const idMessageError = useId();
		...
		try {
			...
			if (!navigator.onLine) {
				setErrors((prev) => ({
					...prev,
					network:
						"Parece que estás sin conexión. Intenta nuevamente cuando tengas conexión.",
				}));
			}
			...
			if (!errors.server && navigator.onLine) {
				setErrors((prev) => ({
					...prev,
					server: "Error al enviar el mensaje. Intenta nuevamente.",
				}));
			}
				...
				<div className="max-w-4xl mx-auto">
					<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
						...
						<div className="space-y-6">
							...
							>
								<h3 className="text-lg font-bold mb-4">
									{t("contact.sidebar.faq")}
								</h3>
								<div className="space-y-4">
									<div>
										<p className="font-medium text-sm mb-1">
											{t("contact.sidebar.faqResponse")}
										</p>
										<p
